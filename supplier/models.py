...
class Supplier(models.Model):
    ...
    # دوال خاصة بالتصنيفات
    def is_driver(self):
        """التحقق من كون المورد سائق - ديناميكي من الإعدادات"""
        if self.primary_type and hasattr(self.primary_type, 'settings') and self.primary_type.settings:
            return self.primary_type.settings.is_driver
        # Fallback للطريقة القديمة
        return self.primary_type and self.primary_type.code == 'driver'
...
class SupplierTypeSettings(models.Model):
    ...
    @property
    def is_driver(self):
        """هل هذا النوع خاص بالسائقين؟"""
        return self.code == 'driver'
    ...
    @classmethod
    def create_school_supplier_types(cls):
        """إنشاء تصنيفات الموردين الخاصة بالشركات"""
        school_types = [
            {
                'name': 'سائق',
                'code': 'driver',
            ...
            },
            {
                'name': 'مورد تعليمي',
                'code': 'educational',
                'description': 'موردي الكتب والمواد التعليمية والقرطاسية',
                'icon': 'fas fa-book',
                'color': '#007bff',
                'display_order': 2,
                'is_system': True
            },
            {
                ...
                'description': 'مقدمي خدمات الصيانة والتنظيف والأمن',
                'icon': 'fas fa-tools',
                'color': '#ffc107',
                'display_order': 3,
                'is_system': True
            },
            {
                'name': 'مورد عام',
                'code': 'general',
                'description': 'الموردين العامين للمواد والخدمات المختلفة',
...
class DriverPricingHistory(models.Model):
    """تاريخ تغييرات تسعيرة السائقين"""
    
    driver_supplier = models.ForeignKey(
        'Supplier',
        on_delete=models.CASCADE,
        related_name='pricing_history',
        verbose_name=_('السائق (مورد)')
    )
    
    old_cost_per_student = models.DecimalField(
        _('التكلفة السابقة للطالب'),
        max_digits=8,
        decimal_places=2,
        null=True,
        blank=True,
        help_text=_('التكلفة الشهرية السابقة للطالب الواحد')
    )
    
    new_cost_per_student = models.DecimalField(
        _('التكلفة الجديدة للطالب'),
        max_digits=8,
        decimal_places=2,
        help_text=_('التكلفة الشهرية الجديدة للطالب الواحد')
    )
    
    effective_date = models.DateField(
        _('تاريخ السريان'),
        help_text=_('تاريخ بداية سريان التكلفة الجديدة')
    )
    
    change_reason = models.TextField(
        _('سبب التغيير'),
        blank=True,
        help_text=_('سبب تغيير التسعيرة')
    )
    ...
    class Meta:
        verbose_name = _('تاريخ تغيير تسعيرة السائق')
        verbose_name_plural = _('تاريخ تغييرات تسعيرة السائقين')
        ordering = ['-effective_date', '-created_at']
        indexes = [
            models.Index(fields=['driver_supplier']),
            models.Index(fields=['effective_date']),
            models.Index(fields=['created_at']),
        ]
    
    def __str__(self):
        old_cost = self.old_cost_per_student or 0
        return f"{self.driver_supplier.name}: {old_cost} ← {self.new_cost_per_student} ({self.effective_date})"


class MonthlyDriverInvoice(models.Model):
    """فواتير السائقين الشهرية"""
    ...
    driver_supplier = models.ForeignKey(
        ...
        verbose_name=_('السائق (مورد)'),
        limit_choices_to={'primary_type__code': 'driver', 'is_active': True}
    )
    ...
    quantity = models.DecimalField(
        ...
        max_digits=6,
        decimal_places=1,
        help_text=_('الكمية أو عدد الوحدات في هذا الشهر')
    )
    ...
    unit_cost = models.DecimalField(
        ...
        max_digits=8,
        decimal_places=2,
        help_text=_('التكلفة الشهرية للوحدة الواحدة وقت إنشاء الفاتورة')
    )
    ...
    total_amount = models.DecimalField(
        ...
        max_digits=10,
        decimal_places=2,
        help_text=_('إجمالي مبلغ الفاتورة (الكمية × التكلفة للوحدة)')
    )
    ...
    purchase_invoice = models.OneToOneField(
        ...
        help_text=_('فاتورة المشتريات المرتبطة في نظام الموردين')
    )
    ...
    notes = models.TextField(
        _('ملاحظات'),
        blank=True,
        help_text=_('ملاحظات إضافية حول الفاتورة')
    )
    ...
    generated_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name=_('أنشأت بواسطة'),
        related_name='monthly_driver_invoices_generated'
    )
    
    class Meta:
        verbose_name = _('فاتورة سائق شهرية')
        verbose_name_plural = _('فواتير السائقين الشهرية')
    ...
    def __str__(self):
        return f"{self.driver_supplier.name} - {self.month}/{self.year} - {self.total_amount}"
    ...
    def clean(self):
        ...
        if self.month and (self.month < 1 or self.month > 12):
            raise ValidationError({
                'month': _('الشهر يجب أن يكون بين 1 و 12')
            })
        
        # التحقق من صحة السنة
        if self.year and (self.year < 2020 or self.year > 2050):
            raise ValidationError({
                'year': _('السنة يجب أن تكون بين 2020 و 2050')
            })
        
        # التحقق من أن السائق من النوع الصحيح
        if self.driver_supplier and (not self.driver_supplier.primary_type or self.driver_supplier.primary_type.code != 'driver'):
            raise ValidationError({
                'driver_supplier': _('يجب أن يكون المورد من نوع "سائق"')
            })
        
        # التحقق من صحة حساب المبلغ الإجمالي
        if (self.quantity is not None and self.unit_cost is not None and 
            ...
            if abs(self.total_amount - expected_total) > Decimal('0.01'):
                raise ValidationError({
                    'total_amount': _('المبلغ الإجمالي غير صحيح. المتوقع: {}').format(expected_total)
                })
    
    def save(self, *args, **kwargs):
        """حفظ الفاتورة مع حساب المبلغ الإجمالي وربطها بالمورد"""
        # حساب المبلغ الإجمالي تلقائياً
        if self.quantity is not None and self.unit_cost is not None:
            self.total_amount = (self.quantity * self.unit_cost).quantize(Decimal('0.01'))
        
        self.full_clean()
        is_new = not self.pk
        
        # التحقق من المعاملة المالية قبل الحفظ للفواتير الجديدة
        if is_new:
            self._validate_financial_transaction()
        
        super().save(*args, **kwargs)
        
        # ربط الفاتورة بالمورد وتحديث الرصيد للفواتير الجديدة
        if is_new:
            self.link_to_supplier_balance()
    
    def _validate_financial_transaction(self):
        """التحقق من المعاملة المالية قبل إنشاء الفاتورة"""
        ...
        try:
            ...
            validation_result = FinancialValidationService.validate_transaction(
                entity=self.driver_supplier,
                transaction_date=timezone.now().date(),
                entity_type='supplier',
                transaction_type='driver_invoice',
                transaction_amount=self.total_amount,
                user=self.generated_by,
                module='supplier',
                view_name='MonthlyDriverInvoice.save',
                raise_exception=True,
                log_failures=True
            )
            
            logger.info(
                f"نجح التحقق من المعاملة المالية لفاتورة السائق {self.driver_supplier.name} "
                f"للشهر {self.month}/{self.year}"
            )
            
        except FinancialValidationError as e:
            logger.error(
                f"فشل التحقق من المعاملة المالية لفاتورة السائق {self.driver_supplier.name}: {str(e)}"
            )
            raise ValidationError({
                'driver_supplier': str(e)
            })
    
    def link_to_supplier_balance(self):
        """ربط الفاتورة برصيد المورد"""
        try:
            # استخدام الطريقة المباشرة
            self._legacy_link_to_supplier_balance()
        except Exception as e:
            logger.error(f'فشل في ربط الفاتورة {self.id} برصيد المورد: {str(e)}')
    
    def _legacy_link_to_supplier_balance(self):
        """الطريقة القديمة لربط الفاتورة برصيد المورد"""
        try:
            supplier = self.driver_supplier
            old_balance = supplier.balance
            supplier.balance += self.total_amount
            supplier.save(update_fields=['balance'])
            
            logger.info(
                f'تم ربط فاتورة {self.id} برصيد المورد {supplier.name}: '
                f'{old_balance} + {self.total_amount} = {supplier.balance}'
            )
            
        except Exception as e:
            logger.error(f'فشل في ربط الفاتورة {self.id} برصيد المورد (الطريقة القديمة): {str(e)}')
    
    @property
    def month_name_ar(self):
        """اسم الشهر بالعربية"""
        months = {
    ...
    def can_be_cancelled(self):
        ...
        return self.status in ['generated', 'confirmed']
    
    def get_payment_status(self):
        """الحصول على حالة الدفع من فاتورة المشتريات"""
        if self.purchase_invoice:
            return {
                'status': 'paid' if self.purchase_invoice.is_fully_paid else 'pending',
                'paid_amount': self.purchase_invoice.total_paid_amount,
                'outstanding_amount': self.purchase_invoice.remaining_amount
            }
        return {
            'status': 'pending',
            'paid_amount': 0,

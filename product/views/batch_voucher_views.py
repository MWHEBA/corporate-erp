...
from product.forms import BatchVoucherForm, BatchVoucherItemFormSet
...
class BatchVoucherCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context['formset'] = BatchVoucherItemFormSet(self.request.POST, instance=self.object, prefix='items')
        else:
            context['formset'] = BatchVoucherItemFormSet(instance=self.object, prefix='items')
        ...
        if formset.is_valid():
            ...
            for f in formset:
                if f.cleaned_data and not f.cleaned_data.get('DELETE', False):
                    has_data = any(v for k, v in f.cleaned_data.items() if k != 'DELETE' and v)
                    if has_data:
                        valid_items.append(f)
            
            # التحقق من وجود منتجات صالحة
            if len(valid_items) == 0:
                messages.error(self.request, 'يرجى إضافة منتج واحد على الأقل')
            ...
            messages.success(self.request, f'تم إنشاء الإذن {self.object.voucher_number} بنجاح')
            return redirect('product:batch_voucher_detail', pk=self.object.pk)
        else:
            messages.error(self.request, 'يرجى تصحيح الأخطاء في النموذج')
            return self.render_to_response(self.get_context_data(form=form))
    
    def get_success_url(self):
        return reverse('product:batch_voucher_detail', kwargs={'pk': self.object.pk})
...
class BatchVoucherUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if formset.is_valid():
            ...
            messages.success(self.request, f'تم تحديث الإذن {self.object.voucher_number} بنجاح')
            return redirect('product:batch_voucher_detail', pk=self.object.pk)
        else:
            messages.error(self.request, 'يرجى تصحيح الأخطاء في النموذج')
            return self.render_to_response(self.get_context_data(form=form))

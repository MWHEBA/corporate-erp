...
- `InventoryMovement` - حركات المخزون المتقدمة (مرتبطة بالأذون الجماعية)
- `InventoryAdjustment` - تسويات المخزون
- `InventoryAdjustmentItem` - بنود تسوية المخزون

**ملاحظة مهمة:** حركات المخزون الفردية يتم إنشاؤها تلقائياً من الأذون الجماعية (`BatchVoucher`) للتتبع والمراجعة التفصيلية.

#### `batch_voucher.py`
**الأذون الجماعية (الواجهة الرئيسية)**
- `BatchVoucher` - الإذن الجماعي (استلام/صرف/تحويل)
- `BatchVoucherItem` - بنود الإذن الجماعي

**النظام الهجين:**
- المستخدم يتعامل مع الأذون الجماعية فقط (واجهة بسيطة)
- النظام ينشئ حركات مخزون فردية تلقائياً (للتتبع والتقارير)
- كل حركة فردية مرتبطة بالإذن الجماعي عبر `batch_voucher` field

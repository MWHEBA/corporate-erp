...
def require_hr(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not (request.user.is_superuser or 
                request.user.groups.filter(name__in=['HR', 'HR Manager']).exists()):
            raise PermissionDenied("يجب أن تكون من HR لتسجيل الأذونات")
...
def can_approve_permissions(view_func):
    """
    يتطلب صلاحية اعتماد الأذونات (المديرين فقط)
    نفس نمط can_approve_leaves
    """
    @wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if not request.user.is_authenticated:

...
def payroll_list(request):
    ...
    for payroll in payrolls:
        ...
        # عرض التصنيف المالي
        if payroll.financial_category:
            payroll.financial_category_display = f'<span class="badge bg-info">{payroll.financial_category.name}</span>'
        else:
...
def payroll_detail(request, pk):
    ...
    # Note: payment_accounts متاح تلقائياً من context processor (لو الراتب معتمد)
...
def advance_request(request):
    ...
    employees = Employee.objects.filter(status='active').select_related('department').order_by('name')
    
    # الحصول على التصنيفات المالية النشطة
    from financial.models import FinancialCategory
    financial_categories = FinancialCategory.objects.filter(is_active=True).order_by('display_order', 'name')

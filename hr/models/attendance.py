...
class Shift(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # حساب ساعات العمل إذا لم تكن محددة أو كانت صفر
        if not self.work_hours or self.work_hours == 0:
            calculated_hours = self.calculate_work_hours()
            if calculated_hours > 0:
                self.work_hours = calculated_hours
